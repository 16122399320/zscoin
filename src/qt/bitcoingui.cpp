...
{
    resize(850, 550);
    setWindowTitle(tr("ZSCOIN") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a ZSCOIN address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About ZSCOIN"), this);
    aboutAction->setToolTip(tr("Show information about ZSCOIN"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for ZSCOIN"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ZSCOIN client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("ZSCOIN client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ZSCOIN network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ZSCOIN address or malformed URI parameters."));
    }
